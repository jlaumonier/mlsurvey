...
class TestFairnessWorkflow(unittest.TestCase):
    ...
    def test_init_should_init(self):
        fw = mls.sl.workflows.FairnessWorkflow(config_directory=self.cd)
        self.assertIsNotNone(fw.config.data)
        self.assertFalse(fw.is_sub_process)
        self.assertIsNone(fw.parent_context)
        self.assertIsInstance(fw.context, mls.sl.models.Context)
    ...
    def test_init_should_init_with_existing_data(self):
        context = mls.sl.models.Context(mls.sl.models.EvaluationSupervised)
        fw = mls.sl.workflows.FairnessWorkflow(context=context)
    ...
    def test_task_get_data_data_is_obtained_with_config(self):
        ...
        """
        fw = mls.sl.workflows.FairnessWorkflow(config_directory=self.cd, base_directory=self.bd)
        fw.task_get_data()
        self.assertIsInstance(fw.context.dataset, mls.sl.datasets.FileDataSet)
    ...
    def test_task_get_data_data_is_obtained_with_context(self):
        ...
        """
        context = mls.sl.models.Context(mls.sl.models.EvaluationSupervised)
        context.dataset = mls.sl.datasets.DataSetFactory.create_dataset('FileDataSet')
        ...
        context.raw_data = mls.sl.models.DataFactory.create_data('Pandas', context.dataset.generate())
        fw = mls.sl.workflows.FairnessWorkflow(context=context)
        fw.task_get_data()
    ...
    def test_task_get_data_no_fairness_data_error_with_config(self):
        ...
        """
        fw = mls.sl.workflows.FairnessWorkflow(config_file='config_fairness_no_fairness_data.json',
    ...
    def test_task_get_data_no_fairness_data_error_with_context(self):
        ...
        dataset_params = {"directory": directory, "filename": "test-fairness.arff"}
        context.dataset.set_generation_parameters(dataset_params)
        context.data = mls.sl.models.DataFactory.create_data('Pandas', context.dataset.generate())
        fw = mls.sl.workflows.FairnessWorkflow(context=context)
        try:
            fw.task_get_data()
            self.assertTrue(False)
    ...
    def test_task_get_data_no_fairness_process_with_config(self):
        ...
        :main_result : mls.exceptions.ConfigError is raised
        """
        fw = mls.sl.workflows.FairnessWorkflow(config_file='complete_config_loaded.json', config_directory=self.cd)
        try:
            fw.task_get_data()
            self.assertTrue(False)
        except mls.exceptions.ConfigError:
    ...
    def test_run_with_real_data(self):
        fw = mls.sl.workflows.FairnessWorkflow(config_file='config_fairness_all_data.json',
                                               config_directory=self.cd,
